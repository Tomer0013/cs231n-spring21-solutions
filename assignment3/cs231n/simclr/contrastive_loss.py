...
def sim(z_i, z_j):
    ...
    norm_dot_product = z_i.dot(z_j) / (z_i.norm(2) * z_j.norm(2))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for kk in range(2*N):
          if kk != k:
            denom += (sim(z_k, out[kk]) / tau).exp()
        l_k_kn = -torch.log(sim_i_j / denom)

        #l(k+N, k)
        denom = 0
        for kk in range(2*N):
          if kk != k+N:
            denom += (sim(z_k_N, out[kk]) / tau).exp()
        l_kn_k = -torch.log(sim_i_j / denom)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left * out_right).sum(dim=1) / (out_left.norm(2, dim=1) * out_right.norm(2, dim=1))
...
def compute_sim_matrix(out):
    ...
    out_norms = out.norm(2, dim=1).view(-1,1)
    norms_matrix = out_norms.matmul(out_norms.permute([1, 0]))
    sim_matrix /= norms_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_positive_pairs_score = sim_positive_pairs(out_left, out_right)
    ...
    numerator = (sim_positive_pairs_score / tau).exp()
    numerator = torch.concat([numerator, numerator], dim=0).to(device)
