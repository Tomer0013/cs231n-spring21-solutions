...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wh, Wx, b = cache

    dnext_h_dx = (1-np.tanh(prev_h.dot(Wh) + x.dot(Wx) + b)**2) * dnext_h  # (N, H)

    dprev_h = dnext_h_dx.dot(Wh.T)  # (N, H)
    dWh = prev_h.T.dot(dnext_h_dx)  # (H, H)
    dWx = x.T.dot(dnext_h_dx)  # (D, H)
    dx = dnext_h_dx.dot(Wx.T)  # (N, D)
    db = np.sum(dnext_h_dx, axis=0)  # (H,)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      x_t = x[:,t,:]
      next_h, cache = rnn_step_forward(x_t, prev_h, Wx, Wh, b)
      hidden_states_list.append(next_h)
      cache_list.append(cache)
      prev_h = next_h
    
    h = np.transpose(np.array(hidden_states_list), axes=[1,0,2])
    cache = cache_list
...
def rnn_backward(dh, cache):
    ...
    for t in range(T):
      dh_t = dh[:,T-t-1,:]
      dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t+dhout_t, cache_list.pop())      
      dhout_t = dprev_h_t     
      dx.append(dx_t) 
      dWx += dWx_t
      dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    a_mat = x.dot(Wx) + prev_h.dot(Wh) + b  # (N, 4H)
 
    i_vec = sigmoid(a_mat[:, :H])
    f_vec = sigmoid(a_mat[:, H:2*H])
    o_vec = sigmoid(a_mat[:, 2*H:3*H])
    g_vec = np.tanh(a_mat[:, 3*H:])
  
    next_c = f_vec * prev_c + i_vec * g_vec
    next_h = o_vec * np.tanh(next_c)

    cache = (x, Wx, Wh, b, prev_c, prev_h, i_vec, f_vec, o_vec, g_vec, next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, Wx, Wh, b, prev_c, prev_h, i_vec, f_vec, o_vec, g_vec, next_c = cache

    dnext_h_dsigmoid_o = dnext_h * np.tanh(next_c)
    dnext_h_dtanh_next_c = dnext_h * o_vec

    dtanh_next_c = (1 - np.tanh(next_c)**2) * dnext_h_dtanh_next_c
    dsigmoid_o = (o_vec * (1-o_vec)) * dnext_h_dsigmoid_o

    dadd = dnext_c + dtanh_next_c

    dadd_dsigmoid_i = dadd * g_vec
    dadd_dtanh_g = dadd * i_vec
    dadd_dsigmoid_f = dadd * prev_c

    dprev_c = dadd * f_vec

    dsigmoid_i = (i_vec * (1-i_vec)) * dadd_dsigmoid_i
    dtanh_g = (1 - g_vec**2) * dadd_dtanh_g
    dsigmoid_f = (f_vec * (1-f_vec)) * dadd_dsigmoid_f

    a_mat = np.zeros((N, 4*H))
    a_mat[:, :H] = dsigmoid_i
    a_mat[:, H:2*H] = dsigmoid_f
    a_mat[:, 2*H:3*H] = dsigmoid_o
    a_mat[:, 3*H:] = dtanh_g
    ...
    dprev_h = a_mat.dot(Wh.T)
    dWh = prev_h.T.dot(a_mat)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      x_t = x[:,t,:]
      next_h, next_c, cache = lstm_step_forward(x_t, prev_h, prev_c, Wx, Wh, b)
      hidden_states_list.append(next_h)
      cache_list.append(cache)
      prev_h = next_h
      prev_c = next_c
...
def lstm_backward(dh, cache):
    ...
    for t in range(T):
      dh_t = dh[:,T-t-1,:]
      dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_t+dhout_t, dcout_t, cache_list.pop())      
      dhout_t = dprev_h_t
      dcout_t = dprev_c_t     
      dx.append(dx_t) 
      dWx += dWx_t
