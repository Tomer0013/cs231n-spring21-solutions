...
class KNearestNeighbor(object):
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            dists[i, :] =  np.sqrt(np.sum(np.power(np.broadcast_to(X[i,:], (num_train, X[i,:].shape[0]))-self.X_train,2), axis=1))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            y_bincount = np.bincount(closest_y)
            y_pred[i] = np.arange(np.max(closest_y)+1)[y_bincount == np.max(y_bincount)][0]
